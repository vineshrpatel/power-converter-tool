...
def display_boost_equations(img, font_size):
    """
    ...
    equations = [
        r"Equations used:",
        r"$D = 1 - \frac{V_{in}}{V_{out}}$",
        r"$I_{load} = \frac{V_{out}}{R_{load}}$",
        r"$L_c = \frac{R_{load} \cdot D \cdot (1-D)^2}{2 \cdot f}$",
        r"$C_{min} = \frac{D \cdot V_{out}}{V_{ripple} \cdot R_{load} \cdot f}$"
    ]
...
def display_boost(Vin, Lc, C_min, R_load, I_load, canvas):
    # Display circuit
    with pkg_resources.path(circuit_diagrams, 'boost_circuit.png') as img_path:
    ...
    draw.text((700, 130), f"{Lc}H", fill=(0, 0, 0), font=font)  # Inductor
    draw.text((1600, 800), f"{C_min}F", fill=(0, 0, 0), font=font)  # Capacitor
    ...
    eq_img = display_boost_equations(eq_img, 10)
    ...
    canvas.create_image(
        canvas.winfo_width()//2,  # Center horizontally
        circuit.height,      # Position below circuit with small gap
        anchor="n",
        image=eq_photo
    )
