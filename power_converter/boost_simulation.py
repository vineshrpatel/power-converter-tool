...
def ltspice_boost_simulation(f, D, Vin, L, C, R):

    AbsPath = os.path.dirname(os.path.realpath(__file__)) 
    # LTC = SimCommander(AbsPath + "\\boost_circuit.asc")

    # Full path to circuit file
    circuit_file = os.path.join(AbsPath, "ltspice_circuits", "boost_circuit.asc")
    raw_file = os.path.join(AbsPath, "ltspice_circuits", "boost_circuit_1.raw")
