...
def display_forward_equations(img, font_size):
    """
    ...
    equations = [
        r"Equations used:",
        r"$D = \frac{V_{\text{out}} \cdot n}{V_{\text{in}}}$",
        r"$I_{\text{load}} = \frac{V_{\text{out}}}{R_{\text{load}}}$",
        r"$L_{\text{crit}} = \frac{(1-D) \cdot R_{\text{load}}}{2 \cdot f}$",
        r"$\Delta I_L = \frac{(V_{\text{in}} - V_{\text{out}}) \cdot D}{f \cdot L}$",
        r"$C = \frac{(1-D)\cdot V_{\text{out}}}{8\cdot \Delta V \cdot L_{\text{crit}}\cdot f^2}$"
    ]
...
def display_forward(Vin, Lc, Lcoupling, C, R_load, I_load, canvas):
    # Display circuit
    with pkg_resources.path(circuit_diagrams, 'forward_circuit.png') as img_path:
        circuit = Image.open(img_path)
    
    # Add values to circuit image
    draw = ImageDraw.Draw(circuit)
    font = ImageFont.truetype("arial.ttf", 80)
    draw.text((50, 940), f"{Vin}V", fill=(0, 0, 0), font=font)  # Voltage source
    draw.text((570, 700), f"{Lc}H", fill=(0, 0, 0), font=font)  # Inductor
    draw.text((960, 430), f"{Lc}H", fill=(0, 0, 0), font=font)  # Inductor
    draw.text((1600, 800), f"{Lcoupling}H", fill=(0, 0, 0), font=font)  # Inductor coupling
    draw.text((2470, 150), f"{Lc}H", fill=(0, 0, 0), font=font)  # Inductor
    draw.text((2600, 770), f"{C}F", fill=(0, 0, 0), font=font)  # Capacitor
    draw.text((3500, 690), f"{R_load}Ω", fill=(0, 0, 0), font=font)  # Load Resistor
    draw.text((3480, 410), f"{I_load}A", fill=(0, 0, 0), font=font)  # Load Current
    ...
    eq_img = display_forward_equations(eq_img, 10)
