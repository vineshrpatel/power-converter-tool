...
def display_zeta_equations(img, font_size):
    """
    ...
    equations = [
        ...
        r"$L1_c = \frac{(1-D)^2 R_{load}}{2Df}$",
        r"$L2_c = \frac{(1-D)^2 R_{load}}{2f}$",
        r"$C1 = \frac{I_{load}D}{\Delta V \cdot f}$",
        r"$C2 = \frac{I_{load}(1-D)}{8 \cdot L2_c \cdot f^2 \Delta V}$",
        r"$\Delta I_L1 = \frac{(V_{out}(1-D)}{L1_c f}$",
        r"$\Delta I_L2 = \frac{(V_{out}(1-D)}{L2_c \cdot f}$",
    ]
...
def display_zeta(Vin, Lc1, C1, Lc2, C2, R_load, I_load, canvas):
    # Display circuit
    with pkg_resources.path(circuit_diagrams, 'zeta_circuit.png') as img_path:
    ...
    draw.text((1300, 920), f"{Lc1}H", fill=(0, 0, 0), font=font)  # Inductor1
    draw.text((1420, 110), f"{C1}F", fill=(0, 0, 0), font=font)  # Capacitor1
    draw.text((2220, 130), f"{Lc2}H", fill=(0, 0, 0), font=font)  # Inductor2
    ...
    eq_img = display_zeta_equations(eq_img, 10)
