...
from power_converter.zeta_calcs import solve_zeta_converter
from power_converter.unit_prefixes import format_value
from power_converter.zeta_circuit import display_zeta
from power_converter.zeta_simulation import ltspice_zeta_simulation
...
def run_cuk_gui():
    ...
    def calculate():
        ...
        try:
            ...
            results_data["User Inputs"] = {
                ...
                "Vripple": f"{format_value(rippleV1)}V",
                "L1": f"{format_value(L1)}H",
                "L2": f"{format_value(L2)}H"
            }

            results = solve_zeta_converter(Vin, Vout, f, R_load, rippleV1, L1, L2)

            # Store calculated results
            results_data["Calculations"] = {
                "D": f"{results['D']}",
                "I_load": f"{results['I_load']}A",
                "L_primary": f"{results['Lc1']}H",
                "L_secondary": f"{results['Lc2']}H",
                "C1": f"{results['C1']}F",
                "C2": f"{results['C2']}F",
            ...
            L1crit_val.configure(text=f"{results['Lc1']}H")
            L1crit_val.grid(row=3, column=1, sticky='w')
            L12crit_val.configure(text=f"{results['Lc2']}H")
            ...
            if L1 is not None or L2 is not None:
                ...
                C2_changed_val.configure(text=f"{results['C2_changed']}A")
                C2_changed_val.grid(row=2, column=3, sticky='w')
                delta_IL1_changed_val.configure(text=f"{results['I_L1_ripple_changed']}A")
                delta_IL1_changed_val.grid(row=3, column=3, sticky='w')
                deltaIL2_changed_val.configure(text=f"{results['I_L2_ripple_changed']}A")
                deltaIL2_changed_val.grid(row=4, column=3, sticky='w')

                results_data["Additional Calculations"] = {
                    "Custom_L1": f"{L1}H",
                    "Custom_L2": f"{L2}H",
                    "IL1_ripple_(for custom L1)": f"{results['I_L1_ripple_changed']}A",
                    "IL2_ripple_(for custom L2)": f"{results['I_L2_ripple_changed']}A"
                }
            else:
            ...
            display_zeta(Vin, L1, results["C1"], L2, results['C2'], R_load, results["I_load"], circuit_canvas)
    ...
    root = ctk.CTk()
    root.title("Zeta Converter Design")
