...
# Rashid
def solve_forward_converter(Vin, Vout, f, n, R_load, rippleV, L):
    # Equations
    D = (Vout*n)/Vin
    I_load = Vout/R_load
    Lcrit = ((1-D)*R_load)/(2*f)
    Ls = Lcrit/(n**2)
    IL_ripple = (Vout * (1-D)) / (f * Lcrit)
    C = ((1-D)*Vout)/(8*rippleV*Lcrit*f**2)
...
# print(solve_forward_converter(25, 20, 0.5, 5e3, 5, 2, None))