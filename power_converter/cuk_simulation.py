...
def ltspice_cuk_simulation(f, D, Vin, L1, C1, L2, C2, R):

    AbsPath = os.path.dirname(os.path.realpath(__file__)) 
    # LTC = SimCommander(AbsPath + "\\boost_circuit.asc")

    # Full path to circuit file
    circuit_file = os.path.join(AbsPath, "ltspice_circuits", "cuk_circuit.asc")
    raw_file = os.path.join(AbsPath, "ltspice_circuits", "cuk_circuit_1.raw")
...
# ltspice_cuk_simulation(80e3, 0.37, 26, 10e-3, 89e-6, 1e-3, 56e-9, 10)
# plt.show()