...
from power_converter.flyback_calcs import solve_flyback_converter
from power_converter.unit_prefixes import format_value
from power_converter.flyback_circuit import display_flyback
from power_converter.flyback_simulation import ltspice_flyback_simulation
...
def run_flyback_gui():
    ...
    def calculate():
        ...
        try:
            ...
            try:
                L_changed = float(entry_L1.get()) if entry_L1.get().strip() else None
            except ValueError:
                L_changed = None
            ...
            results_data["User Inputs"] = {
                ...
                "L": f"{format_value(L_changed)}H"
            }

            results = solve_flyback_converter(Vin, Vout, n, f, R_load, rippleV, L_changed)

            # Store calculated results
            results_data["Calculations"] = {
                "D": f"{results['D']}",
                "I_load": f"{results['I_load']}A",
                "L_primary": f"{results['Lpcrit']}H",
                "L_secondary": f"{results['Lscrit']}H",
                "IL_primary_ripple": f"{results['ripple_I_Lp']}A",
                "IL_secondary_ripple": f"{results['ripple_I_Lp']}A",
                "C": f"{results['C']}F"
            }
            ...
            Lpcrit_label.configure(text=f"Critical Inductance (Primary): ")
            Lpcrit_label.grid(row=3, column=0, sticky='w' ,pady=3)
            Lscrit_label.configure(text=f"Critical Inductance (Secondary): ")
            Lscrit_label.grid(row=4, column=0, sticky='w' ,pady=3)
            deltaILp_label.configure(text=f"Primary Inductor Current Ripple: ")
            deltaILp_label.grid(row=5, column=0, sticky='w' ,pady=3)
            deltaILs_label.configure(text=f"Seconday Inductor Current Ripple: ")
            deltaILs_label.grid(row=6, column=0, sticky='w' ,pady=3)
            C_label.configure(text=f"Capacitance: ")
            C_label.grid(row=7, column=0, sticky='w' ,pady=3)
            ...
            Lpcrit_val.configure(text=f"{results['Lpcrit']}H")
            Lpcrit_val.grid(row=3, column=1, sticky='w')
            Lscrit_val.configure(text=f"{results['Lscrit']}H")
            Lscrit_val.grid(row=4, column=1, sticky='w')
            deltaILp_val.configure(text=f"{results['ripple_I_Lp']}A")
            deltaILp_val.grid(row=5, column=1, sticky='w')
            deltaILs_val.configure(text=f"{results['ripple_I_Ls']}A")
            deltaILs_val.grid(row=6, column=1, sticky='w')
            C_val.configure(text=f"{results['C']}F")
            C_val.grid(row=7, column=1, sticky='w')

            # Only show second set of results if L was provided
            if L_changed is not None:
                L_changed = format_value(L_changed)
                Ls = results['Ls_changed']

                results2_heading.configure(text=f"For L1 = {L_changed}H:")
                results2_heading.grid(row=1, column=2, columnspan=2, sticky='w', pady=5)
                Ls_changed_label.configure(text=f"Secondary Inductance: ")
                Ls_changed_label.grid(row=2, column=2, sticky='w', pady=5)
                deltaILp_changed_label.configure(text=f"Primary Inductor current ripple: ")
                deltaILp_changed_label.grid(row=3, column=2, sticky='w', pady=5)
                deltaILs_changed_label.configure(text=f"Secondary Inductor current ripple: ")
                deltaILs_changed_label.grid(row=4, column=2, sticky='w', pady=5)

                Ls_changed_val.configure(text=f"{results['Ls_changed']}A")
                Ls_changed_val.grid(row=2, column=3, sticky='w')
                deltaILp_changed_val.configure(text=f"{results['ripple_I_Lp_changed']}A")
                deltaILp_changed_val.grid(row=3, column=3, sticky='w')
                deltaILs_changed_val.configure(text=f"{results['ripple_I_Ls_changed']}A")
                deltaILs_changed_val.grid(row=4, column=3, sticky='w')

                results_data["Additional Calculations"] = {
                    "Custom_L": f"{L_changed}H",
                    "Ls_(for custom L)": f"{results['Ls_changed']}H",
                    "ILp_ripple_(for custom L)": f"{results['ripple_I_Lp_changed']}A",
                    "ILs_ripple_(for custom L)": f"{results['ripple_I_Ls_changed']}A"
                }
            else:
                # Clear second set of results if L is not provided
                results2_heading.configure(text="")
                Ls_changed_label.configure(text="")
                deltaILp_changed_label.configure(text="")
                deltaILs_changed_label.configure(text="")

                Ls_changed_val.configure(text="")
                deltaILp_changed_val.configure(text="")
                deltaILs_changed_val.configure(text="")
                
                L_changed = results['Lpcrit']
                Ls = results['Lscrit']

                if "Additional Calculations" in results_data:
                    del results_data["Additional Calculations"]

            display_flyback(Vin, L_changed, Ls, results['C'], R_load, results["I_load"], circuit_canvas)
            ...
            fig = ltspice_flyback_simulation(f,
                                        f"{str(results['D']).replace(' ', '')}",
                                        Vin,
                                        f"{str(results['Lpcrit']).replace(' ', '')}",
                                        f"{str(results['Lscrit']).replace(' ', '')}",
    ...
    root = ctk.CTk()
    root.title("Flyback Converter Design")
    ...
    ctk.CTkLabel(left_frame, text="Turn Ratio: ", font=set_font, height=0).grid(row=4, column=0, sticky='w')
    ...
    ctk.CTkLabel(left_frame, text="Voltage Ripple (V): ", font=set_font, height=0).grid(row=6, column=0, sticky='w')
    ...
    Lpcrit_label = ctk.CTkLabel(right_frame, text="", font=set_font, height=0)
    Lscrit_label = ctk.CTkLabel(right_frame, text="", font=set_font, height=0)
    deltaILp_label = ctk.CTkLabel(right_frame, text="", font=set_font, height=0)
    deltaILs_label = ctk.CTkLabel(right_frame, text="", font=set_font, height=0)
    ...
    Lpcrit_val = ctk.CTkLabel(right_frame, text="", font=set_font, height=0)
    Lscrit_val = ctk.CTkLabel(right_frame, text="", font=set_font, height=0)
    deltaILp_val = ctk.CTkLabel(right_frame, text="", font=set_font, height=0)
    deltaILs_val = ctk.CTkLabel(right_frame, text="", font=set_font, height=0)
    ...
    ctk.CTkLabel(L_frame, text="Primary Inductance (H):", font=set_font, height=0).grid(row=1, column=0, sticky='w')
    ...
    deltaILp_changed_label = ctk.CTkLabel(right_frame, text="", font=set_font, height=0)
    deltaILp_changed_label.grid(row=3, column=2, sticky='w', padx=(40,0))
    deltaILs_changed_label = ctk.CTkLabel(right_frame, text="", font=set_font, height=0)
    deltaILs_changed_label.grid(row=4, column=2, sticky='w', padx=(40,0))

    Ls_changed_val = ctk.CTkLabel(right_frame, text="", font=set_font, height=0)    
    deltaILp_changed_val = ctk.CTkLabel(right_frame, text="", font=set_font, height=0)
    deltaILs_changed_val = ctk.CTkLabel(right_frame, text="", font=set_font, height=0)
