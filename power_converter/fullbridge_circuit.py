...
def display_fullbridge_equations(img, font_size):
    """
    ...
    equations = [
        r"Equations used:",
        r"$D = \frac{V_{\text{out}\cdot n} \cdot n}{2V_{\text{in}}}$",
...
def display_fullbridge(Vin, Lc, Lcoupling, C, R_load, I_load, canvas):
    # Display circuit
    with pkg_resources.path(circuit_diagrams, 'fullbridge_circuit.png') as img_path:
        circuit = Image.open(img_path)
    
    # Add values to circuit image
    draw = ImageDraw.Draw(circuit)
    font = ImageFont.truetype("arial.ttf", 80)
    draw.text((60, 800), f"{Vin}V", fill=(0, 0, 0), font=font)  # Voltage source
    # draw.text((620, 380), "Q1", fill=(0, 0, 0), font=font) 
    # draw.text((1020, 380), "Q3", fill=(0, 0, 0), font=font)
    # draw.text((620, 1230), "Q4", fill=(0, 0, 0), font=font)
    # draw.text((1020, 1230), "Q2", fill=(0, 0, 0), font=font) 
    draw.text((1250, 1070), f"{Lc}H", fill=(0, 0, 0), font=font)  # Inductor
    draw.text((1800, 940), f"{Lcoupling}H", fill=(0, 0, 0), font=font)  # Inductor coupling
    draw.text((2650, 30), f"{Lc}H", fill=(0, 0, 0), font=font)  # Inductor
    draw.text((2760, 600), f"{C}F", fill=(0, 0, 0), font=font)  # Capacitor
    draw.text((3700, 510), f"{R_load}Ω", fill=(0, 0, 0), font=font)  # Load Resistor
    draw.text((3660, 255), f"{I_load}A", fill=(0, 0, 0), font=font)  # Load Current
    ...
    eq_img = display_fullbridge_equations(eq_img, 10)
