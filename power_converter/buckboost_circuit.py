...
def display_buckboost_equations(img, font_size):
    """
    ...
    equations = [
        ...
        r"$L_\text{crit} = \frac{R_{\text{load}} \cdot (1-D)^2}{2 \cdot f}$",
        r"$\Delta I_L = \frac{(V_{\text{in}} \cdot D) \cdot D}{f \cdot L}$",
        r"$I_{\text{SW(max)}} = \Delta I_L$",
    ...
    for equation in equations:
        ...
        new_width = int(equation_image.size[0] * 0.2)  # Scale down if too large
        new_height = int(equation_image.size[1] * 0.2)
...
def display_buckboost(Vin, Lc, C_min, R_load, I_load, canvas):
    # Display circuit
    with pkg_resources.path(circuit_diagrams, 'buck-boost_circuit.png') as img_path:
    ...
    draw.text((850, 680), f"{Lc}H", fill=(0, 0, 0), font=font)  # Inductor
    draw.text((1680, 800), f"{C_min}F", fill=(0, 0, 0), font=font)  # Capacitor
    ...
    eq_img = display_buckboost_equations(eq_img, 10)
