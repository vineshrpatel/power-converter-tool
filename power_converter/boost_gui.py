...
from power_converter.boost_calcs import solve_boost_converter
from power_converter.unit_prefixes import format_value
from power_converter.boost_circuit import display_boost
from power_converter.boost_simulation import ltspice_boost_simulation
...
def run_boost_gui():
    ...
    def calculate():
        ...
        try:
            ...
            results = solve_boost_converter(Vin, Vout, f, R_load, rippleV, L)

            # Store calculated results
            results_data["Calculations"] = {
                "D": f"{results['D']}",
                "I_load": f"{results['I_load']}A",
                "L_crit": f"{results['Lc']}H",
                "C": f"{results['C_min']}F",
                "IL_ripple": f"{results['IL_ripple']}A",
                "I_max_switch": f"{results['I_sw_max']}A",
            }
            ...
            C_label.grid(row=4, column=0, sticky='w', pady=3)
            deltaIL_label.configure(text=f"Inductor current ripple: ")
            deltaIL_label.grid(row=5, column=0, sticky='w', pady=3)
            maxIsw_label.configure(text=f"Maximum switch current: ")
            maxIsw_label.grid(row=6, column=0, sticky='w', pady=3)
            ...
            Lcrit_val.configure(text=f"{results['Lc']}H")
            Lcrit_val.grid(row=3, column=1, sticky='w')
            C_val.configure(text=f"{results['C_min']}F")
            C_val.grid(row=4, column=1, sticky='w')
            deltaIL_val.configure(text=f"{results['IL_ripple']}A")
            deltaIL_val.grid(row=5, column=1, sticky='w')
            maxIsw_val.configure(text=f"{results['I_sw_max']}A")
            maxIsw_val.grid(row=6, column=1, sticky='w')
            ...
            if L is not None:
                ...
                deltaIL_changed_label.grid(row=2, column=2, sticky='w', pady=5)
                maxIsw_changed_label.configure(text=f"Maximum switch current: ")
                maxIsw_changed_label.grid(row=3, column=2, sticky='w', pady=5)

                deltaIL_changed_val.configure(text=f"{results['Lripple2']}A")
                deltaIL_changed_val.grid(row=2, column=3, sticky='w')
                maxIsw_changed_val.configure(text=f"{results['Maxsw2']}A")
                maxIsw_changed_val.grid(row=3, column=3, sticky='w')

                results_data["Additional Calculations"] = {
                    "Custom_L": f"{L}H",
                    "IL_ripple_(for custom L)": f"{results['Lripple2']}A",
                    "I_max_switch_(for custom L)": f"{results['Maxsw2']}A"
                }
            else:
            ...
            display_boost(Vin, L, results["C_min"], R_load, results["I_load"], circuit_canvas)
            ...
            fig = ltspice_boost_simulation(f,
                                        f"{str(results['D']).replace(' ', '')}",
                                        Vin,
                                        f"{str(L).replace(' ', '')}", 
                                        f"{str(results['C_min']).replace(' ', '')}",
                                        R_load)
    ...
    root = ctk.CTk()
    root.title("Boost Converter Design")
    ...
    ctk.CTkLabel(left_frame, text="Ripple Voltage (V): ", font=set_font, height=0).grid(row=5, column=0, sticky='w')
    entry_rippleV = ctk.CTkEntry(left_frame, font=set_font, height=0, width=80, border_width=1, corner_radius=3)
    entry_rippleV.grid(row=5, column=1, pady=1)
    ...
    deltaIL_changed_label.grid(row=2, column=2, sticky='w', padx=(40,0))
    maxIsw_changed_label = ctk.CTkLabel(right_frame, text="", font=set_font, height=0)
    maxIsw_changed_label.grid(row=3, column=2, sticky='w', padx=(40,0))
