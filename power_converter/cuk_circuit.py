...
def display_cuk_equations(img, font_size):
    """
...
def display_cuk(Vin, Lc1, C1, Lc2, Cout, R_load, I_load, canvas):
    # Display circuit
    with pkg_resources.path(circuit_diagrams, 'cuk_circuit.png') as img_path:
    ...
    draw.text((685, 130), f"{Lc1}H", fill=(0, 0, 0), font=font)  # Inductor1
    draw.text((1420, 110), f"{C1}F", fill=(0, 0, 0), font=font)  # Capacitor1
    draw.text((2220, 130), f"{Lc2}H", fill=(0, 0, 0), font=font)  # Inductor2
    draw.text((2300, 800), f"{Cout}F", fill=(0, 0, 0), font=font)  # Ouput Capacitor
    ...
    eq_img = display_cuk_equations(eq_img, 10)
