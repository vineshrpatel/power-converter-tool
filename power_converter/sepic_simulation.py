...
def ltspice_sepic_simulation(f, D, Vin, L, C1, C2, R):

    AbsPath = os.path.dirname(os.path.realpath(__file__)) 
    # LTC = SimCommander(AbsPath + "\\boost_circuit.asc")

    # Full path to circuit file
    circuit_file = os.path.join(AbsPath, "ltspice_circuits", "sepic_circuit.asc")
    raw_file = os.path.join(AbsPath, "ltspice_circuits", "sepic_circuit_1.raw")
    ...
    LTC.set_component_value('C1', str(C1))
    LTC.set_component_value('L2', str(L))
    LTC.set_component_value('C2', str(C2))
...
# ltspice_sepic_simulation(80e3, 0.37, 26, 1e-3, 89e-6, 56e-9, 10)
# plt.show()