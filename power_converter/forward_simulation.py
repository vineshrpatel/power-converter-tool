...
def ltspice_forward_simulation(f, D, Vin, Lc, Lcoupling, C, R):

    AbsPath = os.path.dirname(os.path.realpath(__file__)) 
    # LTC = SimCommander(AbsPath + "\\boost_circuit.asc")

    # Full path to circuit file
    circuit_file = os.path.join(AbsPath, "ltspice_circuits", "forward_circuit.asc")
    raw_file = os.path.join(AbsPath, "ltspice_circuits", "forward_circuit_1.raw")
    ...
    LTC.set_parameter('L', str(Lc))
    LTC.set_component_value('L2', str(Lcoupling))
...
# ltspice_forward_simulation(80e3, 0.37, 26, 10e-3, 89e-6, 10)