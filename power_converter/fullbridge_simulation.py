...
def ltspice_fullbridge_simulation(f, D, Vin, Lc, Lcoupling, C, R):

    AbsPath = os.path.dirname(os.path.realpath(__file__)) 
    # LTC = SimCommander(AbsPath + "\\boost_circuit.asc")

    # Full path to circuit file
    circuit_file = os.path.join(AbsPath, "ltspice_circuits", "fullbridge_circuit.asc")
    raw_file = os.path.join(AbsPath, "ltspice_circuits", "fullbridge_circuit_1.raw")
    ...
    LTC.set_component_value('Vin', str(Vin))
    ...
    LTC.set_component_value('V1', f'PULSE(0 5 0 0 0 {ton} {T})') # Format: PULSE(V1 V2 Tdelay Trise Tfall Ton Period)
    LTC.set_component_value('V2', f'PULSE(0 5 0 0 0 {ton} {T})')
    LTC.set_component_value('V3', f'PULSE(0 5 {ton} 0 0 {T-ton} {T})')
    LTC.set_component_value('V4', f'PULSE(0 5 {ton} 0 0 {T-ton} {T})')
    ...
    time =  l.get_time()
    Vout = l.get_data("V(n008)")
    I_load = l.get_data("I(R1)")
    I_L = l.get_data("I(L4)")
    I_switch1 = l.get_data("Id(M1)")
    I_switch2 = l.get_data("Id(M3)")
    ...
    ax5.set_xlabel('Time')
    ax5.set_ylabel('$\\mathrm{I_{switch3}}$', fontsize=12)
...
# ltspice_fullbridge_simulation(12e3, 0.109, 55, 81.439e-6, 904.882e-6, 20.833e-6, 10)