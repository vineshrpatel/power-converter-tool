...
from power_converter.buck_calcs import solve_buck_converter
from power_converter.unit_prefixes import format_value
from power_converter.buck_circuit import display_buck
from power_converter.buck_simulation import ltspice_buck_simulation
...
def run_buck_gui():
    ...
    def calculate():
        ...
        try:
            ...
            results = solve_buck_converter(Vin, Vout, f, R_load, rippleV, L)
            ...
            if L is not None:
                ...
                deltaIL_changed_label.grid(row=2, column=2, sticky='w')
                maxIsw_changed_label.configure(text=f"Maximum switch current: ")
                maxIsw_changed_label.grid(row=3, column=2, sticky='w')
                ...
                C_changed_val.configure(text=f"{results['C2']}F")
                ...
                results_data["Additional Calculations"] = {
                    ...
                    "C_(for custom L)": f"{results['C2']}F"
                }
            else:
            ...
            display_buck(Vin, L, results["C"], R_load, results["I_load"], circuit_canvas)
    ...
    root = ctk.CTk()
    root.title("Buck Converter Calculator")
